...
function displayTasks(tasks) {
    ...
    tasks.forEach((task) => {
        ...
        console.log(`${task.id} | ${task.description} | ${task.status} | ${createdDate} | ${updatedDate}`);
    });
}
// Process commands
    ...
    try {
        switch (command) {
            case "add":
                ...
                const addResult = taskService_1.TaskService.addTask(args[1]);
            ...
            case "update":
                ...
                const updateResult = taskService_1.TaskService.updateTask(taskId, args[2]);
            ...
            case "delete":
                ...
                const deleteResult = taskService_1.TaskService.deleteTask(deleteId);
            ...
            case "mark-in-progress":
                ...
                const inProgressResult = taskService_1.TaskService.updateTaskStatus(inProgressId, task_1.TaskStatus.IN_PROGRESS);
                if (inProgressResult.success) {
                    console.log(`Task ${inProgressId} marked as in-progress`);
                }
                else {
                    console.error(`Failed to update task status: ${inProgressResult.message}`);
                }
                break;
            case "mark-done":
                if (!args[1]) {
                ...
                const doneResult = taskService_1.TaskService.updateTaskStatus(doneId, task_1.TaskStatus.DONE);
            ...
            case "list":
                ...
                switch (filter) {
                    case "todo":
                        tasks = taskService_1.TaskService.listTasks(task_1.TaskStatus.TODO);
                        console.log("Tasks To Do:");
                        break;
                    case "in-progress":
                        tasks = taskService_1.TaskService.listTasks(task_1.TaskStatus.IN_PROGRESS);
                        console.log("Tasks In Progress:");
                        break;
                    case "done":
                        tasks = taskService_1.TaskService.listTasks(task_1.TaskStatus.DONE);
                        console.log("Taks Done:");
                        break;
                    default:
                        tasks = taskService_1.TaskService.listTasks();
